...
export default function Page() {
  ...
  useEffect(() => {
    async function checkConnectivity() {
      // Note: diagnoseSdkConnectivity may not be available in all Sentry versions
      // const result = await Sentry.diagnoseSdkConnectivity();
      const result = { status: 'Sentry SDK initialized' };
      setIsConnected(result.status !== 'sentry-unreachable');
    }
    checkConnectivity();
  }, []);

  // Inject styles only on client to prevent hydration mismatch
  useEffect(() => {
    const styleId = 'sentry-example-styles';
    ...
    <div>
      <main>
        ...
        <p className="description">
          Click the button below, and view the sample error on the Sentry <a target="_blank" href="https://chatflow.sentry.io/issues/?project=4510472158773328">Issues Page</a>.
